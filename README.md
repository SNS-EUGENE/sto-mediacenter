...
| 경로 | 설명 |
|------|------|
| `/` | 메인 대시보드 - 오늘의 예약, 가동률, 수익 현황 |
| `/bookings` | 예약 관리 - 목록 조회, 검색, 필터링, 상세 보기 |
| `/calendar` | 캘린더 뷰 - 월간/주간 예약 일정 |
| `/equipments` | 장비 관리 - 인벤토리, 상태 추적 |
| `/statistics` | 통계 - 가동률 분석, 장기 이용자 현황 |
| `/surveys` | 만족도 조사 관리 - 응답 목록, 통계 |
| `/kpi` | KPI 대시보드 - 월별 목표 및 달성률 |
| `/settings` | 시스템 설정 - STO 연동, 알림 수신자 |
| `/live` | 실시간 모니터 - 스튜디오 현황 라이브 뷰 |
| `/kiosk` | 키오스크 모드 - 방문객용 예약 확인 화면 |
...
|------|------|
| `/login` | 로그인 페이지 |
| `/survey/[token]` | 만족도 조사 응답 (토큰 기반 익명 설문) |
| `/survey/today` | 당일 설문 (PIN 인증) |
| `/surveys/today` | 당일 설문 (대체 경로) |
